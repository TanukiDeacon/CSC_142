...
int main(){
    ...
    // width variables
    std::string kitWid , livWid, bedWid;
    double kitchenWidth;
    double livingRoomWidth;
    double bedRoomWidth;

    // length variables
    std::string kitLen, livLen, bedLen;
    double kitchenLength;
    double livingRoomLength;
    double bedRoomLength;


    std::cout << "What is the width of the kitchen: ";
    std::getline(std::cin, kitWid);
    kitchenWidth = std::stod(kitWid);
    std::cout << "What is the length of the kitchen: ";
    std::getline(std::cin, kitLen);
    kitchenLength = std::stod(kitLen);

    std::cout << "What is the width of the living room: ";
    std::getline(std::cin, livWid);
    livingRoomWidth = std::stod(livWid);
    std::cout << "What is the length of the living room: ";
    std::getline(std::cin, livLen);
    livingRoomLength = std::stod(livLen);

    std::cout << "What is the width of the bedroom: ";
    std::getline(std::cin, bedWid);
    bedRoomWidth = std::stod(bedWid);
    std::cout << "What is the length of the bedroom: ";
    std::getline(std::cin, bedLen);
    bedRoomLength = std::stod(bedLen);


    kitchen.setWidth(kitchenWidth);
    kitchen.setLength(kitchenLength);
    std::cout << "\nThe area of the kitchen is "<< kitchen.getArea() << std::endl;

    livingRoom.setWidth(livingRoomWidth);
    livingRoom.setLength(livingRoomLength);
    std::cout << "The area of the living room is "<< livingRoom.getArea() << std::endl;

    bedRoom.setWidth(bedRoomWidth);
    bedRoom.setLength(bedRoomLength);
    std::cout << "The area of the bedroom is "<< bedRoom.getArea() << std::endl;

    double total = bedRoom.getArea() + livingRoom.getArea() + kitchen.getArea();

    std::cout << "\nThe total of all three rooms is " << total << std::endl;
